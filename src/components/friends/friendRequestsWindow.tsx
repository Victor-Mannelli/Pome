...
import { acceptFriendRequest, getAllUsers, getFriendRequests } from '@/utils/functions';
...
export function FriendRequestsElement({ setShowFriendRequests, showFriendRequests, userId, data }: {
  ...
  const receivedFR = friendRequests.filter((e: FriendRequests) => e.requested_id === userId);
  const friends = [
      ...
      {receivedFR.length > 0 ?
        <>
          <FaUserFriends
            ...
            onClick={() => { setShowFriendRequests(!showFriendRequests); setUpdate(!update) }}
          />
