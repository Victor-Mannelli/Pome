  ...
  return (
    ...
    >
      ...
      >
        ...
        <Link href={'/'}>
          <h1 className="text-lg cursor-pointer hover:text-signature transition-all">
            {' '}
            ...
            {user ? (
              <HoverList>
                <Link href={`/profile/${user.user_id}`}>
                  <div className="hover:bg-fifth w-full px-4 py-2 cursor-pointer text-white">Profile</div>
                </Link>
                <Link href={'/'} onClick={() => logout({ setToken, setUser, toast })}>
                  <div className="hover:bg-fifth w-full px-4 py-2 cursor-pointer text-white">Log out</div>
                </Link>
              </HoverList>
            ) : (
              // <HoverList>
              <div className={'hover:flex flex-col items-end right-0 top-[2.2rem] -z-[1] bg-transparent pt-7 w-[6rem] iconDescription'}>
                ...
                <div className="bg-second py-2 rounded-md rounded-tr-none">
                  <Link href={'/login'}>
                    <div className="hover:bg-fifth w-full px-6 py-1 cursor-pointer text-white">Log in</div>
                  </Link>
                </div>
              </div>
              // </HoverList>
