    ...
    <div className={`relative flex justify-start w-full min-h-72 h-fit ${animeData.bannerImage ? "" : "mt-16"}`}>
      ...
      >
        <Image
          id="coverImage"
          alt="coverImage"
          className="float-start w-60 h-[23.5rem] rounded-lg"
      ...
      <div id="animeInfo" className="flex flex-col w-full m-5 md:pl-0 xl:overflow-auto">
        <h1 id="title" className={`w-full pr-7 font-bold text-2xl ${animeData.bannerImage ? "md:pl-[17.75rem]" : ""} `}> {animeData.title.romaji} </h1>
        <div className="flex flex-col lg:flex-row pt-3 gap-7">
            ...
            {!animeData.genres ? null :
              <div id='genres' className="flex flex-wrap gap-1 xl:w-[33.5rem] xx:w-fit">
                {animeData.genres.map((e: string, i: number) => (
                  <h3 key={e + i} className="w-32 h-9 rounded-lg text-center pt-1 text-eigth text-md border"> {e} </h3>
                ))}
              </div>
            }
          </div>
          {!animeData.tags ? null :
            <div
              id="tags"
              className={`flex ${animeData.bannerImage ? "xl:flex-col flex-wrap" : "flex-col"}  items-start gap-2 pl-2 md:pl-7 lg:pl-0 xx:pl-36 xl:h-52 xx:h-48  xl:w-[28rem]`}
            >
