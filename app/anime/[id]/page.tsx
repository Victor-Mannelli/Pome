...
export default function AnimePage({ params }: { params: { id: string; }; }) {
  ...
  const { loading, error: dataFailed, data } = useQuery<{ Media: SingleAnimeDataForSlug; }>(
    getAnimeQuery, 
      ...
      {dataFailed ? (
        ...
        <div className="flex flex-col items-center w-full mb-5">
          {data.Media.bannerImage ? (
            <div className={"w-full h-80 bg-cover bg-center"} style={{ backgroundImage: `url(${data.Media.bannerImage})` }}>
              {" "}
            </div>
          ) : null}
          <AnimeInfo
          ...
          {data.Media.trailer && data.Media.trailer.site === "youtube" ? (
            <div className="relative bg-fourthAndAHalf rounded-xl w-[calc(100%-40px)] mt-5 p-5">
              ...
              <div className="relative flex justify-center">
                <iframe className={`${trailerFullScreen ? "w-[94%] h-[calc(100vh-7.5rem)]" : "h-[25rem] w-[40rem]"}`} src={`https://www.youtube.com/embed/${data.Media.trailer.id}`} allowFullScreen />
              </div>
